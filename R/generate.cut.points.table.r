    ...
    cutpoints_mtrx <- do.call(cbind, lapply(seq_along(cut_points), function (iter){
        ...
        sapply(1:len_plus_one, function (jiter){
            ...
            ## For subsequent cut points and
                ...
                return_object <- paste(round(cut_points[[iter]][jiter - 1],
                                                            digits=digits),
                                                      "to",
                                                      round(cut_points[[iter]][jiter],
                                                            digits = digits))
                if (to_invert) return_object <- paste(round(cut_points[[iter]][jiter],
                                                             digits=digits),
                                                       "to",
                                                       round(cut_points[[iter]][jiter - 1],
                                                             digits = digits))
            ## For last cut point
