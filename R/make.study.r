...
make.study <- function(data_path = "./extdata/test_sample.csv",
    ...
    ## Error handling
    if (!(is.character && length(data_path) == 1)) stop("The data_path has to be a string.")
    if (!(bootstrap_samples%%1 == 0) | !is.numeric(boostrap_samples)) stop("Number of bootstrap samples must be integer.")
    if (!(is.logical(test))) stop("Variable test has to be of type logical.")
    # Read study data from data_path
